...
public partial class XS_SXZJList : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from  V_周记 where 学号='"+Session["UserID"].ToString()+"' and 职位 like '%"+TextBox1.Text+"%' ");
        if (dt != null)
        {
            this.gvData.DataKeyNames = new string[] { "周记编号" };
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "SXZJ.aspx?ID=" + gvData.DataKeys[num]["周记编号"].ToString();
            PageBase.OpenWebForm(strUrl);
        }
        else if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from 实习周记 where 周记编号=" + gvData.DataKeys[num]["周记编号"].ToString() + "");
            BindResult();
        }
    }
