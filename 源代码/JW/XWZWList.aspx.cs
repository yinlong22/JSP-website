...
public partial class JW_XWZWList : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from  V_安排 where 单位类别='XW' and 单位名称 like '%"+TextBox1.Text+"%' and 学号 like '%"+TextBox2.Text+"%'  ");
        if (dt != null)
        {
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "XWZW.aspx?ID=" + gvData.DataKeys[num]["安排编号"].ToString();
            PageBase.OpenWebForm(strUrl);
        }
        else if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from 实习安排 where 安排编号=" + gvData.DataKeys[num]["安排编号"].ToString() + "");
            BindResult();
        }
    }
