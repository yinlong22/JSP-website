...
public partial class JW_DWList : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from  单位 where 单位名称 like '%"+TextBox1.Text+"%' and 单位类别='"+Request.QueryString["LB"]+"' ");
        if (dt != null)
        {
            this.gvData.DataKeyNames = new string[] { "单位编号" };
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "DW.aspx?ID=" + gvData.DataKeys[num]["单位编号"].ToString() + "&LB=" + Request.QueryString["LB"];
            PageBase.OpenWebForm(strUrl);
        }
        else if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from 单位 where 单位编号=" + gvData.DataKeys[num]["单位编号"].ToString() + "");
            BindResult();
        }
    }
    ...
    {
        Response.Redirect("DW.aspx?LB=" + Request.QueryString["LB"]);
    }
}
