...
public partial class Admin_FBTJ : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select 单位类别,(case 单位类别 when 'XN' then '校内' else '校外' end) as 分布情况,count(1) as 实习生人数 from V_安排  group by 单位类别  ");
        if (dt != null)
        {
            this.gvData.DataKeyNames = new string[] { "单位类别" };
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "SXSSee.aspx?LB=" + gvData.DataKeys[num]["单位类别"].ToString();
