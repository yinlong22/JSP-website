...
public partial class Admin_RYList : System.Web.UI.Page
{
 protected void Page_Load(object sender, EventArgs e)
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from  人员 where 帐号 like '%"+TextBox1.Text+"%' ");
        if (dt != null)
        {
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "RY.aspx?ID=" + gvData.DataKeys[num]["帐号"].ToString();
            PageBase.OpenWebForm(strUrl);
        }
        else if (e.CommandName == "Del")
        {
            DataControl.Execute("delete from 人员 where 帐号='" + gvData.DataKeys[num]["帐号"].ToString() + "'");
            BindResult();
        }
    }
