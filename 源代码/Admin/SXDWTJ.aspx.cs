...
public partial class Admin_SXDWTJ : System.Web.UI.Page
{
    ...
    private void BindResult()
    {
        ...
        //组合查询条件 
        DataTable dt = DataControl.GetData("select * from 单位 left join (select 单位编号,count(1) as 实习生人数 from V_安排 group by 单位编号) A  on 单位.单位编号=A.单位编号 ");
        if (dt != null)
        {
    ...
    protected void gvData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string strUrl = "SXSSee.aspx?DWID=" + gvData.DataKeys[num]["单位编号"].ToString();
